...
export function useTenantPlan() {
  ...
  const query = useQuery({
      ...
      return {
        ...
        usage: {
          vehicles: {
            ...
            max: tenant.plans.max_vehicles || Infinity,
            canAdd: tenant.plans.max_vehicles === 0 || vehiclesCount < tenant.plans.max_vehicles,
            percentage: tenant.plans.max_vehicles === 0 ? 0 : Math.round((vehiclesCount / tenant.plans.max_vehicles) * 100),
          },
          users: {
            current: usersCount,
            max: tenant.plans.max_users || Infinity,
            canAdd: tenant.plans.max_users === 0 || usersCount < tenant.plans.max_users,
            percentage: tenant.plans.max_users === 0 ? 0 : Math.round((usersCount / tenant.plans.max_users) * 100),
          },
          contracts: {
            current: contractsCount,
            max: tenant.plans.max_contracts || Infinity,
            canAdd: tenant.plans.max_contracts === 0 || contractsCount < tenant.plans.max_contracts,
            percentage: tenant.plans.max_contracts === 0 ? 0 : Math.round((contractsCount / tenant.plans.max_contracts) * 100),
          },
          clients: {
            current: clientsCount,
            max: tenant.plans.max_clients || Infinity,
            canAdd: tenant.plans.max_clients === 0 || clientsCount < tenant.plans.max_clients,
            percentage: tenant.plans.max_clients === 0 ? 0 : Math.round((clientsCount / tenant.plans.max_clients) * 100),
          },
        },
        modules: {
    ...
    staleTime: 5 * 60 * 1000, // Cache 5 minutes (les quotas ne changent pas souvent)
