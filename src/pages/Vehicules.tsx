...
export default function Vehicules() {
  ...
  const filteredVehicles = vehicles.filter(vehicle => {
    ...
    if (advancedFilters.searchText) {
      ...
      const matchesSearch = vehicle.immatriculation?.toLowerCase().includes(searchLower) || vehicle.immatriculation_provisoire?.toLowerCase().includes(searchLower) || vehicle.marque?.toLowerCase().includes(searchLower) || vehicle.modele?.toLowerCase().includes(searchLower) || vehicle.categorie?.toLowerCase().includes(searchLower);
  ...
  const handleExport = (exportFormat: 'excel' | 'csv') => {
    const data = prepareVehiclesExport();
    const filename = `vehicules_${format(new Date(), 'yyyy-MM-dd')}`;
    ...
    toast({
      title: 'Export réussi',
      description: `${filteredVehicles.length} véhicule(s) exporté(s) en ${exportFormat.toUpperCase()}`,
    });
  };
                    ...
                    <CardContent className="p-4">
                      <div className="flex items-start justify-between mb-3">
                        ...
                        <div className="flex flex-col gap-1">
                          {getTypeVehiculeBadge(vehicle)}
                          {getStatusBadge(vehicle)}
                          {vehicle.type_vehicule !== 'sous_location' && needsOilChange(vehicle) && <Badge variant="outline" className={`${getOilChangeAlertLevel(vehicle) === 'critical' ? 'bg-red-500/10 text-red-600 border-red-500/20' : 'bg-orange-500/10 text-orange-600 border-orange-500/20'} text-xs`}>
                              🛠️ Vidange
                            </Badge>}
                        </div>
                      </div>
